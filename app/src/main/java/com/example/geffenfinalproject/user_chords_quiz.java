...
public class user_chords_quiz extends BaseActivity {
    ...
    private Button btnPlayChord, btnCheckAnswer, btnStop;
    ...
    private void highlightSelectedRoot() {
        for (int i = 0; i < 12; i++) {
            if (i == selectedRootIndex) {
                ...
                rootButtons[i].setBackgroundColor(getColor(R.color.black)); // Or some other highlight
            } else {
    ...
    private void generateAndPlayChord() {
        ...
        playChord(correctRootIndex, correctIsMinor);
    }

    private void playChord(int rootIdx, boolean isMinor) {
        stopAudio();

        int octave = 3 + random.nextInt(2); // Use octave 3 or 4
        ...
        // Play them with small delays for a "strummed" effect or all at once
    ...
    private int getNoteResId(int noteIdx, int octave) {
        ...
        // Handle A0, A#0, B0 and C8 edge cases if necessary, but 3-5 is safe
        int resId = getResources().getIdentifier(prefix + octave, "raw", getPackageName());
        if (resId == 0) {
            // Fallback to a safe octave if not found
    ...
    private void stopAudio() {
        if (mpRoot != null) { mpRoot.release(); mpRoot = null; }
        if (mpThird != null) { mpThird.release(); mpThird = null; }
        if (mpFifth != null) { mpFifth.release(); mpFifth = null; }
    }
